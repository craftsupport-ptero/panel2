...
exports.bulkServerAction = bulkServerAction;
exports.migrateServer = migrateServer;
exports.getOptimizationSuggestions = getOptimizationSuggestions;
exports.backupAllServers = backupAllServers;
exports.getPerformanceAnalysis = getPerformanceAnalysis;
const zod_1 = require("zod");
// Server bulk action schema
const BulkServerActionSchema = zod_1.z.object({
    action: zod_1.z.enum(['start', 'stop', 'restart', 'kill', 'backup', 'reinstall']),
    filters: zod_1.z.object({
        node_id: zod_1.z.number().optional(),
        user_id: zod_1.z.number().optional(),
        status: zod_1.z.string().optional(),
        server_ids: zod_1.z.array(zod_1.z.number()).optional()
    }).optional(),
...
const ServerMigrationSchema = zod_1.z.object({
    server_id: zod_1.z.number(),
    target_node_id: zod_1.z.number(),
    options: zod_1.z.object({
 ...
 */
async function getServerOverview(req, res) {
    try {
 ...
 */
async function bulkServerAction(req, res) {
    try {
 ...
 */
async function migrateServer(req, res) {
    try {
 ...
 */
async function backupAllServers(req, res) {
    try {
        if (!req.user?.hasPermission('admin.servers.bulk_operations')) {
            return res.status(403).json({
                error: 'Insufficient permissions'
...
async function getPerformanceAnalysis(req, res) {
    try {
        ...
        const { timeframe = '24h' } = zod_1.z.object({
