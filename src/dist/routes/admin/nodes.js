...
exports.getNodeOverview = getNodeOverview;
exports.setNodeMaintenance = setNodeMaintenance;
exports.drainNode = drainNode;
exports.getCapacityPlanning = getCapacityPlanning;
exports.optimizeNodeAllocation = optimizeNodeAllocation;
const zod_1 = require("zod");
// Node maintenance schema
const NodeMaintenanceSchema = zod_1.z.object({
    node_id: zod_1.z.number(),
    maintenance_mode: zod_1.z.boolean(),
    reason: zod_1.z.string().optional(),
    estimated_duration: zod_1.z.number().optional(), // minutes
    allow_new_servers: zod_1.z.boolean().default(false)
});
// Node drain schema
const NodeDrainSchema = zod_1.z.object({
    node_id: zod_1.z.number(),
    target_nodes: zod_1.z.array(zod_1.z.number()).optional(),
    options: zod_1.z.object({
        graceful: zod_1.z.boolean().default(true),
        timeout: zod_1.z.number().default(300), // seconds
 ...
 */
async function getNodeOverview(req, res) {
    try {
 ...
 */
async function setNodeMaintenance(req, res) {
    try {
        ...
        const { node_id, maintenance_mode, reason, estimated_duration, allow_new_servers } = NodeMaintenanceSchema.parse(req.body);
 ...
 */
async function drainNode(req, res) {
    try {
        if (!req.user?.hasPermission('admin.nodes.manage')) {
            return res.status(403).json({
                error: 'Insufficient permissions'
 ...
 */
async function getCapacityPlanning(req, res) {
    try {
        if (!req.user?.hasPermission('admin.nodes.view')) {
            return res.status(403).json({
                error: 'Insufficient permissions'
        ...
        const { timeframe = '30d' } = zod_1.z.object({
            timeframe: zod_1.z.enum(['7d', '30d', '90d', '1y']).default('30d')
        }).parse(req.query);
 ...
 */
async function optimizeNodeAllocation(req, res) {
    try {
        if (!req.user?.hasPermission('admin.nodes.manage')) {
            return res.status(403).json({
                error: 'Insufficient permissions'
        ...
        const { strategy = 'balanced', dry_run = true } = zod_1.z.object({
